...
-- From person table, emailpromotion <> 0, where <> means not equal to 0
...
-- From employee table, vacationhours >= 99, where >= means more than or equal to
...
--Conditions are now applied together
--
...
SELECT 
	...
	COALESCE(SUM(salesorderdetail.orderqty),0) AS totalsalesqty
...
SELECT 
CONCAT(person.firstname, ' ', person.middlename, ' ', person.lastname) AS fullname, emailaddress.emailaddress AS email
...
SELECT
	...
	CURRENT_DATE + INTERVAL '10 days' add_days,
	CURRENT_DATE - INTERVAL '10 days' minus_days,
	CURRENT_DATE + INTERVAL '1 month' add_month
FROM sales.salesorderheader
WHERE territoryid = 1
...
SELECT 
	...
	LENGTH(person.lastname) as firstnamelength,
